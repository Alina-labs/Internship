...
const SignIn=()=> {
  ...
  const handleLogin =async ()=>{
    ...
    try {
      const response = await fetch('http://localhost:8000/api/auth/csignin', {
        method: 'POST',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2" style={{backgroundColor:"" }}>
        <div className="flex items-center justify-center px-4 py-10 sm:px-6 sm:py-16 lg:px-8 lg:py-24" >
          <div className="xl:mx-auto xl:w-full xl:max-w-sm 2xl:max-w-md">
            <h2 className="text-3xl font-bold leading-tight text-green-700 sm:text-4xl">Sign in</h2>
            ...
            <form action="#" method="POST" className="mt-8">
              <div className="space-y-5">
                ...
                <div>
                  <button
                    type="button"
                    className="inline-flex w-full items-center justify-center rounded-md bg-green-800 px-3.5 py-2.5 font-semibold leading-7 text-white hover:bg-black/80"
                    onClick={handleLogin}  
                  >
